...
async function startResearcherSearch() {
  if (!location.href.includes("https://jglobal.jst.go.jp/detail?JGLOBAL_ID=")) {
    alert("この拡張機能はJ-GLOBALの研究者詳細ページでのみ使用できます。");
    return;
  }

  const major = prompt("研究者の専門分野を書いてください:");
  await searchResearcherAndSendMessage(major);
}

async function searchResearcherAndSendMessage(major) {
  const professor = document.querySelector(
    "h1.search_detail_topbox_title"
  ).textContent;
  console.log(professor);
  const contact = document.querySelector("button.btn_l.btn_mail");

  await waitForUrlChange("https://jglobal.jst.go.jp/mail/researcher?");
  chrome.storage.local.set({ major, professor });
    contact.click();
}

// async function populateForm(major, professor) {
...
//   const messageTextarea =
//     professor +
//     "先生\n突然のご連絡を失礼いたします、株式会社PRESの森田と申します。\nこのたびは〇〇先生に、企業が公開するWeb記事の監修についてご相談したく、ご連絡させていただいております。\n弊社は「研究者の方々が研究のみに没頭できる世界を作る」を理念とした新興企業(スタートアップ)です。投資機関・銀行から1.2億円を調達し、いくつかの産学連携マッチング事業を立ち上げました。\nこの事業を通じ、なるべく多くの研究資金を研究者の方々へお渡しすることで、国内研究業界の課題の一つと言われる研究資金不足を解決しようと挑戦しております。\nこのたびは、" +
//     major +
//     "学分野に関するWeb記事の監修にご協力いただける先生を探しており、大学・研究室HPや科研費データベースなどから、" +
//     professor +
//     "先生を拝見し、ご連絡させていただいております。\n監修依頼料についてですが、1記事につき「月額1万円×記事の掲載期間」をお支払いいたします。\n通常1社あたり3-4記事単位でご依頼いただいており、またもしご迷惑でなければ本件以外にも" +
...
// URLが変わるのを待つ関数
function waitForUrlChange(targetUrl) {
  const conditionFunction = () => location.href.includes(targetUrl);
  const poll = (resolve) => {
